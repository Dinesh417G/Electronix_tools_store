...
function ItemScreen({
  ...
  // with a burst of parent renders (the poll dropping the just-claimed card,
  // the revision bump, the connection pill going live), so the restart landed
  // inside the window and the stale handle blanked the live stream. First
  // scan of every session: a black rectangle. Held in a ref instead, so the
  // running scanner always calls the current one.
