...
//
// A copy of `cloud/tests/cdp.mjs`, because the two halves of the workspace are
// separate npm packages and neither can import from the other. Everything it
// knows was learned on the cloud side — the launch flags, the port discovery,
// the reason `stdio: "ignore"` was a mistake — and none of it is worth
// learning twice.
//
// What a headless browser gives that `fetch` cannot: real React state and the
// actual click handlers the shop floor will be tapping.
//
